...
The main puprose of this sample is provide a good starting point for applications that would like to use the "A/B" update method.
...
The sample uses several manifest-controlled variables:
 ...
 - ``BOOT_PREFERENCE``: This variable is used to pass information from the application to the SDFW about the intended variant to be booted. The root manifest is allowed to boot the other variant if the preferred one failed.
...
The sample logic for managing confirmation variables can be found in ``src/device_management.c`` file.
...
It is important to note a few rules, that were followed while wrtiting the memory map:
 ...
 - A small staging area, called ``dfu_partition`` was defined.
   The size of it is set to 32kB, so it is capable of holding all update candidate manifests, but is insufficient to store firmware binaries.
...
This is done by adding ``#include "nrf54h20dk_nrf54h20_memory_map.dtsi"`` inside:
...
The second part of the sample configuration are SUIT manifests.
...
The logic inside the suit-payload-fetch sequence is sensitive to the set of available integrated manifests.
...
Each root manifest contains both variants of the application and radio manifests.
Based on the ``BOOT_STATUS`` manifest variable value, one set of manifests is selected.
...
The logic inside the suit-candidate-verification is very similar to the logic inside suit-payload-fetch sequence.
...
The logic inside the suit-install is very similar to the logic inside suit-candidate-verification sequence.
The main differences are:

 - The suit-install sequence from the local manifests is executes.
 ...
 - The logic updates the ``CONFIRM_SET_A`` or ``CONFIRM_SET_B`` before executing suit-install sequences of the local manifests.
 - The logic updates the ``BOOT_PREFERENCE`` after executing suit-install sequences of the local manifests.
...
After the reboot, the SDFW will proceed with the boot path, executing suit-validate sequence of the new root manifest.
...
The last part of the sample configuration is the Manifest Provisioning Information (MPI).
...
LED 3 and 4:
    These LEDs indicate the status of manifests inside the device:
     ...
     - If only LED 3 blinks, the application is booted without radio capabilities.
